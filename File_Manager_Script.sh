...
create_file()
{
 read -p "Enter the file name to create: " file_name
 if [[ -e $file_name ]]; then
...
create_directory()
{
 read -p "Enter the directory name to create: " directory_name
...
delete_directory()
{
 read -p "Enter the directory name you want to delete: " directory_name
 if [[ -d $directory_name ]]; then
 rm -r "$directory_name"
...
*) echo "Invalid choice please eneter a number between 1 to 6";;
esac
done